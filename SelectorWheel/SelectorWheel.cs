...
namespace SelectorWheel
{
    ...
    public class Wheel
    {
        ...
        public void CalculateCategoryPlacement()
        {
            ...
            if (!HaveTexturesBeenCached)
            {
                foreach (var cat in Categories)
                {
                    ...
                    {
                        cat.CategoryTexture = new Texture(TexturePath + UIHelper.MakeValidFileName(cat.Name) + ".png", Categories.IndexOf(cat));
                    }
                    foreach (var item in cat.ItemList)
                    {
                        if (File.Exists(TexturePath + UIHelper.MakeValidFileName(item.Name) + ".png"))
                        {
                            item.ItemTexture = new Texture(TexturePath + UIHelper.MakeValidFileName(item.Name) + ".png", Categories.IndexOf(cat) /*cat.ItemList.IndexOf(item)*/);
                        }
                    }
        ...
        void ControlCategorySelection()
        {
            ...
            if (SelectedCategory.SelectedItem.Description != null)
            {
                ...
                UIHelper.DrawCustomText(SelectedCategory.SelectedItem.Description, 0.35f, GTA.Font.ChaletLondon, 255, 255, 255, 255, pixelX, pixelY, UIHelper.TextJustification.Left, true, pixelX, 1250f / (float)UI.WIDTH, true, 0, 0, 0, 180, 10f / (float)UI.WIDTH, 10f / (float)UI.HEIGHT);
            }
        ...
        void ControlTransitions()
        {
            if (transitionIn)
            {
                float tempTScale = DecreaseNum(timeScale, Game.LastFrameTime * 8f, 0.05f);
                Game.TimeScale = tempTScale;
                timeScale = tempTScale;

                float tempStrength = IncreaseNum(timecycleCurrentStrength, Game.LastFrameTime * 8f, TimecycleModifierStrength);
                Function.Call(Hash.SET_TIMECYCLE_MODIFIER_STRENGTH, tempStrength);
                timecycleCurrentStrength = tempStrength;
            }
            if (transitionOut)
            {
                float tempTScale = IncreaseNum(timeScale, Game.LastFrameTime * 8f, 1f);
                Game.TimeScale = tempTScale;
                timeScale = tempTScale;
    ...
    public class WheelCategoryItem
    {
        ...
        //public float DescriptionWidth { get; protected set; }
        //public float DescriptionFontHeight { get; protected set; }
        ...
        public string Description
        {
            ...
            set
            {
                /*if (value != null)
                {
                    DescriptionWidth = UIHelper.MeasureStringWidth(value, GTA.Font.ChaletComprimeCologne, 0.49f);
                    DescriptionFontHeight = UIHelper.MeasureFontHeight(0.49f, GTA.Font.ChaletComprimeCologne);
                }*/
